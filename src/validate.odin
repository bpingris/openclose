...
validate_prd :: proc(prd_path: string) -> []Validation_Error {
	...
	for section in required_sections {
		if !strings.contains(text, section) {
			append(&errors, Validation_Error{file = "PRD.md", message = fmt.tprintf("Missing required section: %s", section)})
		}
	}
	...
	if len(lines) > 0 {
		...
		if !strings.has_prefix(first_line, "# ") {
			append(&errors, Validation_Error{file = "PRD.md", message = "First line should be a title (e.g., # Spec Name)"})
		}
	} else {
...
validate_tasks :: proc(tasks_path: string) -> []Validation_Error {
	...
	for line in lines {
		...
		if strings.has_prefix(trimmed, "- [ ]") || strings.has_prefix(trimmed, "- [x]") || strings.has_prefix(trimmed, "- [X]") {
			has_checkboxes = true
		}
	}
	
	if !has_phases {
		append(&errors, Validation_Error{file = "tasks.md", message = "Missing phase sections (e.g., ## Phase 1: Foundation)"})
	}
	
	if !has_checkboxes {
		append(&errors, Validation_Error{file = "tasks.md", message = "No task checkboxes found (format: - [ ] Task description)"})
	}
...
validate_scenarios :: proc(scenarios_path: string) -> []Validation_Error {
	...
	if !has_given {
		append(&errors, Validation_Error{file = "scenarios.md", message = "No Given clauses found"})
	}
	if !has_when {
